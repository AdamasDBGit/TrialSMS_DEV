...
BEGIN
	...
	if(@CatagoryID>0)
	BEGIN
	if exists (select * from [SMS].[dbo].[T_ERP_Fee_Structure_Category] where [S_Fee_Structure_Catagory_Name]  = @CatagoryName and [I_Fee_Structure_Catagory_ID] != @CatagoryID)
	BEGIN
	SELECT 0 StatusFlag,'Duplicate Fee Catagory Name' Message
	END
	ELSE
	BEGIN
	update [SMS].[dbo].[T_ERP_Fee_Structure_Category]
	set 
	...
	ELSE
	BEGIN
	if exists (select * from [SMS].[dbo].[T_ERP_Fee_Structure_Category] where [S_Fee_Structure_Catagory_Name]  = @CatagoryName)
	BEGIN
	SELECT 0 StatusFlag,'Duplicate Fee Catagory Name' Message
	END
	ELSE
	BEGIN
	INSERT INTO [SMS].[dbo].[T_ERP_Fee_Structure_Category]
(
